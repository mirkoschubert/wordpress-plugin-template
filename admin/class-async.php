...
abstract class Async {
   ...
   * @var string
   */
  protected $action = PLUGIN_SLUG . '_async';
  ...
  protected function capability() {
    ...
    if (!Helpers::current_user_can()) {
      ...
      $this->send_response(
        false,
        __('You are not allowed to perform this action. Please contact site administrator for further information.', PLUGIN_SLUG)
      );
  ...
  protected function referer() {
    ...
    if (check_ajax_referer($token, 'nonce', false) === false) {
      ...
      $this->send_response(
        false,
        __('An error occurred. Please try to refresh the page or logout and login again.', PLUGIN_SLUG)
      );
  ...
  protected function unknown_error() {
    ...
    $this->send_response(
      false,
      __('Sorry, an unknown error occurred.', PLUGIN_SLUG)
    );
